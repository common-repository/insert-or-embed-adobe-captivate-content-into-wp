...
$cap_UploadsPath=cap_getUploadsPath();
	...
	else
	{
		...
		if(cap_has_CPLibraryAll_css($dir))
		{
			#check for index.html or index.htm
			$has_index=false;
			foreach($fileList as $fname){ if($fname== "index_AICC.html" || $fname== "index.html" || $fname== "index.htm" || $fname== "index_SCORM.html" || $fname== "index_scorm.html" || $fname== "index_aicc.html" || $fname== "index_tincan.html" || $fname== "index_TINCAN.html"){$has_index=true; $entryName=$fname; break; } }
			if('has_index')
			{
				return $entryName;
			}
...
if (count($dirs)>1)
{
echo '<span style="color:#ff0000; font-size:15px;">The trial version only supports two uploads. Please purchase the full version with unlimited uploads at <a href="https://www.elearningfreak.com/presenter/insert-or-embed-articulate-content-into-wordpress-plugin-premium/" target="_blank">www.elearningfreak.com</a></span>';	
}
else
{	
?>
...
<p><i>Please choose a .zip file that you published with the Adobe Captivate software | <a href="https://www.elearningfreak.com/presenter/insert-or-embed-articulate-content-into-wordpress-plugin-premium/" target="_blank">Maximum upload file size</a>: <strong><?php echo $upload_size_unit; echo $sizes[$u]; ?></strong></i></p>
...
function wp_ajax_cap_check_folder($dir){
...
function cap_extractZip($fileName,$target,$dir)
{
    ...
    if ($res === TRUE) 
	{
         ...
         $zip->close();
		#check if quiz is in a folder as many times as it takes    @anthonysbrown
		 wp_ajax_cap_check_folder($target);
